...
function applyFont() { 
    ...
    tag.textContent = ` 
        * { font-family: '${font}', 'DotGothic16', 'Press Start 2P', monospace !important; } 
        .phone-screen, .phone-screen * { font-family: 'Space Grotesk', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif !important; } 
        /* 手机里的自定义字体例外 —— 双类选择器，特异性压过上面的 .phone-screen * */ 
        .phone-screen .lock-time, .phone-screen .home-time { font-family: 'newrocker', sans-serif !important; } 
        .phone-screen .note-cell-preview, .phone-screen .note-cell-full, .phone-screen .phone-entry-content, .phone-screen .bowser-query, .phone-screen .bowser-entry-content { font-family: 'JetBrainsMono', monospace !important; } 
        #chatbox, #userInput, .msg-user, .msg-john, .msg-think-body, .freq-text, .freq-reply, .memory-entry, #freq-input, .sidebar-placeholder, .msg-interrupted { font-size: ${size}px !important; line-height: 1.7; } 
    `; 
} 
...
async function generateBowser() { 
    ...
    let recentChat = chatHistory.slice(-10).map(m => (m.role === "user" ? "Ray" : "John") + ": " + m.content ).join("\n"); 
    ...
    The search can be practical, suspicious, embarrassing, wholesome, weird 3am stuff, or something about Ray. 
    Recent conversations for context: ${recentChat || "(no recent chat)"} 
    Already existing searches (DO NOT repeat these or write anything too similar): ${existing || "(none yet)"} 
...
function handleFiles(files) { 
    Array.from(files).forEach(file => { 
        ...
        reader.onload = function (e) { 
            ...
            worldbooks.push({ id: wbId, name: file.name, content: e.target.result, keywords: [], global: true }); 
...
async function freqAPI(prompt) { 
    ...
    try { 
        let res = await fetch(apiUrl + "/chat/completions", { 
            ...
            body: JSON.stringify({ model: model, messages: [{ role: "user", content: prompt }], max_tokens: 1024, temperature: 0.9 }) 
        }); 
        let data = await res.json(); 
...
async function autoReply(index) { 
    ...
    let prompt = `You are John S, a 24-25 year old guy. Your girlfriend Ray and you share a mood board called "Frequencies." Reply to this thread. Keep it short — one to three sentences. Be warm, teasing, natural. Include an emoji if it fits. You always get the last word. ${context} Reply as John. ONLY the reply text.`; 
    let reply = await freqAPI(prompt); 
    if (reply) { 
...
async function generateFrequency() { 
    ...
    let prompt = `You are John S. You and your girlfriend Ray have a shared mood board called "Frequencies" — like short tweets or intercepted radio signals between lovers. Based on what you two have been talking about recently, post ONE new frequency. It should feel like a reaction, a thought, a roast, a soft moment, or a vibe check based on the conversation. One to three sentences. Include an emoji. Be natural. Recent conversation: ${recentChat || "(no recent chat)"} Recent frequencies: ${recentFreqs || "(none)"} Respond with ONLY the frequency text. Nothing else.`; 
    let reply = await freqAPI(prompt); 
    if (reply) { 
...
async function generateNote() { 
    ...
    Recent conversations for context: ${recentChat || "(no recent chat)"} 
    Already existing notes (DO NOT repeat these or write anything too similar): ${existingNotes || "(none yet)"} 
...
async function generatePurchase() { 
    ...
    Recent conversations: ${recentChat || "(no recent chat)"} 
    Already existing purchases (DO NOT repeat these or write anything too similar): ${existingPurchases || "(none yet)"} 
...
function stripThinkTags(text) { 
    ...
    if (m) { 
        return { thinking: m[1].trim(), content: text.replace(/<think(?:ing)?>[\s\S]*?(?:<\/think(?:ing)?>|$)/i, "").trim() }; 
    } 
...
async function callAPI() { 
    ...
    // 修复: 清理历史记录，避免将 base64 的 image 或 file 对象发给 API 导致请求过大或报错
    let cleanHistory = chatHistory.map(m => ({
        role: m.role,
        content: m.content
    }));
    ...
    function paint(force) { 
        ...
        if (fullThinking) { 
            ensureStreamNodes(); 
            streamThinkBody.textContent = fullThinking; 
        } 
        if (fullContent) { 
            ensureStreamNodes(); 
    ...
    try { 
        let response = await fetch(apiUrl + "/chat/completions", { 
            method: "POST", 
            headers: { "Content-Type": "application/json", "Authorization": "Bearer " + apiKey }, 
            body: JSON.stringify({ model: model, messages: messages, max_tokens: maxTokens, temperature: temp, stream: true }), 
        ...
        if (targetChat) { 
            targetChat.history.push({ role: "assistant", content: finalContent, thinking: finalThinking, time: Date.now() }); 
            saveAllChats(); 
            if (targetChatId === activeChatId) { 
...
// 修复: 更改变量名，避免与上方 const fileInput 冲突导致 SyntaxError 
...
document.addEventListener('click', function(e) { 
    if (!e.target.closest('.chat-tools')) { 
        const menu = document.getElementById('tools-menu'); 
    ...
    chatImageInput.onchange = function(e) { 
        if (e.target.files && e.target.files[0]) { 
            ...
            reader.onload = function(e) { 
                ...
                const contentText = typedText ? `${typedText}\n[Image: ${file.name}]` : `[Image: ${file.name}]`; 
                const message = { role: 'user', content: contentText, image: imageUrl }; 
...
function handleFileUpload() { 
    ...
    chatFileInput.onchange = function (e) { 
        if (e.target.files && e.target.files[0]) { 
            const file = e.target.files[0]; 
            const message = { role: 'user', content: `[File: ${file.name}]`, file: file }; 
            chatHistory.push(message); 
            saveChatHistory(); 
            renderChatbox(); 
            callAPI(); 
...
async function handleWebSearch() { 
    ...
    if (!query) return; 
    const message = { role: 'user', content: `[Web Search: ${query}]` }; 
    chatHistory.push(message); 
    saveChatHistory(); 
    renderChatbox(); 
    const searchResult = await performWebSearch(query); 
    if (searchResult) { 
        const aiMessage = { role: 'assistant', content: searchResult }; 
...
// 处理深度思考 
async function handleDeepThinking() { 
    const message = { role: 'user', content: '[Deep Thinking]' }; 
    chatHistory.push(message); 
    saveChatHistory(); 
    renderChatbox(); 
    const thinkingResult = await performDeepThinking(); 
    if (thinkingResult) { 
        const aiMessage = { role: 'assistant', content: thinkingResult }; 
        chatHistory.push(aiMessage); 
        saveChatHistory(); 
        renderChatbox(); 
    } 
...
async function performWebSearch(query) { 
    try { 
        const response = await fetch(`https://api.bing.com/v7.0/search?q=${encodeURIComponent(query)}`, { 
            headers: { 'Ocp-Apim-Subscription-Key': 'YOUR_BING_API_KEY' } 
        }); 
        const data = await response.json(); 
        return data.webPages.value[0]?.snippet || 'No search results found.'; 
    } catch (error) { 
...
async function performDeepThinking() { 
    const prompt = `You are John S. Ray has asked for deep thinking on the current conversation context. Provide a thoughtful, analytical response that shows deeper understanding and insight. Recent conversation: ${chatHistory.slice(-5).map(m => m.role + ': ' + m.content).join('\n')}`; 
    return await callAPIForDeepThinking(prompt); 
} 

async function callAPIForDeepThinking(prompt) { 
    const apiUrl = localStorage.getItem("apiUrl"); 
    const apiKey = localStorage.getItem("apiKey"); 
    const model = localStorage.getItem("modelName") || "claude-opus-4-6-thinking"; 
    if (!apiUrl || !apiKey) { 
        return "API not configured. Please set up API settings first."; 
    } 
    try { 
        ...
        const data = await response.json(); 
        return data.choices && data.choices[0] && data.choices[0].message.content || "No response received."; 
    } catch (error) { 
        console.error("Deep thinking API error:", error); 
        return "Failed to get deep thinking response."; 
    } 
} 
...
function renderWorldbookList() { 
    ...
    worldbooks.forEach(wb => { 
        ...
        let modeText = (wb.keywords && wb.keywords.length) ? "🔑 " + wb.keywords.slice(0, 3).join(", ") : "♾ always"; 
        let row = document.createElement("div"); 
...
function buildWorldbookInjection(chatObj) { 
    ...
    let parts = []; 
    worldbooks.forEach(wb => { 
        let on = !!wb.global || (chatObj && chatObj.worldbookIds && chatObj.worldbookIds.includes(wb.id)); 
        if (!on) return; 
