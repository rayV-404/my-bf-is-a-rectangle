// ===== SETTINGS =====
let deepThinkingOn = localStorage.getItem("deepThinkingOn") === "1";
...
function applyFont() {
    ...
    if (!tag) { tag = document.createElement("style"); tag.id = "dynamic-font-style"; document.head.appendChild(tag); }
    tag.textContent = `
        * { font-family: '${font}', 'DotGothic16', 'Press Start 2P', monospace !important; }
        ...
        .phone-screen .note-cell-preview, .phone-screen .note-cell-full,
        .phone-screen .phone-entry-content, .phone-screen .bowser-query,
        .phone-screen .bowser-entry-content { font-family: 'JetBrainsMono', monospace !important; }
        #chatbox, #userInput, .msg-user, .msg-john, .msg-think-body, .freq-text, .freq-reply,
...
const bowserLoadingPhases = [
    "opening bowser...", "loading cookies...", "scrolling history...",
    "found something...", "clearing evidence..."
];
...
async function generateBowser() {
    ...
    let prompt = `You are looking at John S's search history on his phone. John is 24-25, plays drums, wears all black, drinks iced americanos and white Monsters, loves Deftones and horror movies, dates Ray (calls her Kitty). His girlfriend Ray just opened his browser and is snooping.
Generate ONE search history entry. Return in this EXACT format:
...
function deletePreset() {
    ...
    if (idx === "") { alert("select a preset first"); return; }
...
async function fetchModels() {
    ...
    if (!apiUrl || !apiKey) { alert("fill in API URL and key first"); return; }

    let btn = document.querySelector(".fetch-btn");
    btn.textContent = "📡 ...";
    btn.disabled = true;
    try {
        let res = await fetch(apiUrl + "/models", { headers: { "Authorization": "Bearer " + apiKey } });
...
if (fileArea && fileInput) {
    ...
    fileArea.addEventListener("dragover", (e) => { e.preventDefault(); fileArea.style.borderColor = "#7c3aed"; });
    fileArea.addEventListener("dragleave", () => { fileArea.style.borderColor = "#555"; });
    fileArea.addEventListener("drop", (e) => {
        e.preventDefault(); fileArea.style.borderColor = "#555";
...
function saveAllChats() {
    try {
        ...
        alert("⚠️ 存储空间满了（多半是图片太多），较早的图片可能没保存下来。建议导出备份后清理聊天记录。");
    }
}
...
function switchChat(id) {
    ...
    awaitingReply = false;   // 切窗口时清掉待回复状态，防止小字挂错地方
    saveAllChats();
    renderChatList();
    renderChatbox();
    if (isMobile()) showMobileView('chat');
}
...
function deleteChat(id, e) {
    if (e) e.stopPropagation();
    if (allChats.length <= 1) { alert("can't delete your only chat"); return; }
...
function renderChatList() {
    ...
    allChats.forEach(chat => {
        ...
        if (chat.created) {
            ...
            else dateKey = monthShort[d.getMonth()] + " " + d.getDate();
        } else {
            dateKey = "older";
        }
...
function renderFrequencies() {
    ...
    document.querySelectorAll(".freq-reply-input").forEach(input => {
        input.addEventListener("keydown", function (e) {
            if (e.key === "Enter") submitFreqReply(parseInt(this.dataset.index), this.value);
        });
    });
...
async function generateFrequency() {
    let apiUrl = localStorage.getItem("apiUrl");
    let apiKey = localStorage.getItem("apiKey");
    if (!apiUrl || !apiKey) { alert("set up API first ⚙️"); return; }
...
async function callAPI() {
    ...
    function ensureStreamNodes() {
        ...
        if (targetChatId === activeChatId) chatbox.appendChild(streamWrapper);
    }
    ...
    // ↓↓↓ 三选一，留能用的那个，删掉其它两行：
    if (!deepThinkingOn) {
        bodyObj.enable_thinking = false;              // Qwen3 / vLLM / SGLang 系中转
        // bodyObj.thinking = { type: "disabled" };   // Anthropic Claude 系中转
        // bodyObj.reasoning = { exclude: true };     // OpenRouter 风格：允许思考但不下发
    }
    ...
    try {
        ...
        if (targetChat) {
            ...
            awaitingReply = false;   // 回复成功到达，等待状态结束
...
// 最后一条是没有被回复的用户消息（新发后被中止的、或刚编辑过的）→ 直接重发
async function resendLastMessage() {
    if (isGenerating) return;
    let last = chatHistory[chatHistory.length - 1];
    if (!last || last.role !== "user") return;
    awaitingReply = true;
    await callAPI();   // 历史末尾本来就是（编辑后的）用户消息，callAPI 会原样发出去
}
...
// 用户的个人习惯：Shift+Enter 发送、Enter 换行（保持原样不动）
...
document.getElementById("chatbox").addEventListener("click", function (e) {
    ...
    if (actionBtn) {
        ...
        if (action === "delete") {
            ...
            // 删完以后如果历史不再以"未被回复的用户消息"结尾，清掉待回复状态
            let lastM = chatHistory[chatHistory.length - 1];
            if (!lastM || lastM.role !== "user") awaitingReply = false;
            saveChatHistory();
            renderChatbox(true);
        } else if (action === "edit") {
            editMessage(index);
...
document.querySelectorAll('.tool-item').forEach(item => {
    item.addEventListener('click', function () {
        ...
        if (action === 'deep-thinking') {
            ...
            localStorage.setItem("deepThinkingOn", deepThinkingOn ? "1" : "0");
            this.classList.toggle("tool-active", deepThinkingOn);
        } else {
...
document.querySelector('.tool-item[data-action="deep-thinking"]')
    ?.classList.toggle("tool-active", deepThinkingOn);

async function handleToolAction(action) {
    switch (action) {
        case 'upload-image': handleImageUpload(); break;
        case 'upload-file': handleFileUpload(); break;
        case 'web-search': handleWebSearch(); break;
    }
}
...
function handleFileUpload() {
    ...
    chatFileInput.onchange = function (e) {
        ...
        reader.onload = function (ev) {
            chatHistory.push({
                role: "user",
                content: `[用户上传了文件 ${file.name}] 文件内容如下：\n${ev.target.result}`,
                time: Date.now()
            });
            saveChatHistory();
            awaitingReply = true;
        ...
        reader.readAsText(file);   // 只适合文本类文件
    };
}

// 处理联网搜索：先拿到结果再写进历史，失败不留脏消息
async function handleWebSearch() {
    ...
    chatHistory.push({ role: 'user', content: `[Web Search: ${query}]`, time: Date.now() });
    chatHistory.push({ role: 'assistant', content: searchResult || "Search failed.", time: Date.now() });
    saveChatHistory();
    renderChatbox();
}
...
function saveWbEditor() {
    ...
    if (!name || !content) { alert("name and content cannot be empty"); return; }
...
function returnSectionsToSidebar() {
    ...
    ["freq-section", "music-section", "phone-section", "memory-section"].forEach(id => {
        ...
        if (el && el.parentElement !== sidebar) {
            if (mobileNav) { sidebar.insertBefore(el, mobileNav); }
...
function openMemoryPanel() {
    if (isMobile()) { showMobileView("memory"); }
    else {
